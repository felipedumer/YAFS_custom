...
def create_application():
    ...
    m_aa = Message("M.A", "Sensor", "ServiceA", instructions=20 * 10**6, bytes=1000)
    m_b = Message(
        "M.B",
        "ServiceA",
        "Actuator",
    ...
    a.add_service_module("ServiceA", m_aa, m_bb, fractional_selectivity, threshold=1.0)
...
def main(simulated_time):
    ...
    t.load(t_json)
    nx.write_gexf(
        t.G, folder_results + "graph_main2.gexf"
    ...
    pop.set_sink_control(
        {"model": "actuator-device", "number": 2, "module": app.get_sink_modules()}
    )
