...
def create_application():
    ...
    m_a2 = Message("M.A2", "Sensor", "ServiceA", instructions=20 * 10**6, bytes=1000)
...
def main(simulated_time):
    ...
    t.load(t_json)
    nx.write_gexf(
        t.G, folder_results + "graph_main1"
    ...
    placement = CloudPlacement(
        "onCloud"
    )  # it defines the nice rules: module-device
    ...
    dDistribution = deterministic_distribution(name="Deterministic", time=50)
    pop.set_src_control(
        {
            "model": "sensor-device",
            "number": 1,
    ...
    )  # To test deployments put test_initial_deploy a TRUE
