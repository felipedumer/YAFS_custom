...
def create_random_topology(
    ...
    link_pr_cloud_proxy=2,         # Simulator is broken, it's serializing each message, so every message has to wait this simulation time, bandwidth doesn't exist.
    # 10000 Mbps ≈ 1_250_000_000 bytes per second
    ...
    link_pr_edge_device=2,         # Simulator is broken, it's serializing each message, so every message has to wait this simulation time, bandwidth doesn't exist.
):
    """
